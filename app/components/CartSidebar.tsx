      ...
      <div className={`fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[92%] max-w-lg bg-white rounded-[2.5rem] z-[250] overflow-hidden transform transition-all duration-500 shadow-2xl ${isCartOpen ? "scale-100 opacity-100" : "scale-95 opacity-0 pointer-events-none"}`}>
        <div className="p-6 md:p-8 flex flex-col max-h-[85vh]">
          ...
          <div className="flex-grow overflow-y-auto space-y-4 mb-6 pr-2" style={{ maxHeight: '40vh' }}>
            {cartItems.length === 0 ? (
              <p className="text-center text-zinc-400 py-10 uppercase tracking-widest text-[10px]">Vacío</p>
            ) : (
              cartItems.map((item, index) => (
                <div key={index} className="flex justify-between items-center bg-zinc-50 p-4 rounded-3xl border border-zinc-100">
                  ...
                  <div className="flex items-center gap-4">
                    <p className="text-zinc-900 font-black">${item.price.toFixed(2)}</p>
                    <button onClick={() => removeFromCart(item.id)} className="text-zinc-300 hover:text-brand-red">
                      <Trash2 size={18} />
                    </button>
                  </div>
                </div>
          ...
          <div className="border-t border-zinc-100 pt-6">
            ...
            <button disabled={cartItems.length === 0} className="w-full bg-brand-red text-white py-5 rounded-2xl font-black uppercase italic hover:bg-zinc-900 transition-all active:scale-95 disabled:opacity-30">
              Finalizar Pedido
            </button>
          </div>
        </div>
